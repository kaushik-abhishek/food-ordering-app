...
const Header = () => {
  ...
  return (
    <div className="sticky top-0 z-50 flex justify-between items-center glass shadow-sm px-8 py-4 transition-all duration-300">
      <div className="flex items-center gap-8">
        <Link to="/" className="flex-shrink-0">
          <img className="w-12 hover:scale-105 transition-transform" src={LOGO_URL} alt="Logo" />
        </Link>
        <Location />
      </div>
      <div className="flex items-center">
        <ul className="hidden lg:flex items-center gap-2">
          <li className="px-4 font-medium text-gray-700 text-lg hover:text-primary transition duration-200 flex items-center group">
            <Link to="/search" className="flex items-center">
              <FontAwesomeIcon icon={faMagnifyingGlass} size="sm" className="group-hover:scale-110 transition-transform" />
          ...
          <li className="px-4 font-medium text-gray-700 text-lg hover:text-primary transition duration-200">
            <Link to="/" className="flex items-center group">
              ...
              <span className="font-semibold">
                Offers<sup className="m-1 text-primary font-bold">New</sup>
              </span>
            </Link>
          </li>
          <li className="px-4 font-medium text-gray-700 text-lg hover:text-primary transition duration-200">
          ...
          <li className="px-4">
            <Link to="/cart" className="flex items-center group px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-full transition duration-300">
              <span className="flex items-center">
                ...
                {/* <span className="_2vS77 ml-1">{cartItems.length} -</span> */}
                <span className="ml-1 font-bold text-gray-800">Cart</span>
              </span>
              {/* Cart - ({cartItems.length} items) */}
            </Link>
          </li>
          <button
            className="ml-4 px-6 py-2 font-bold text-white bg-primary hover:bg-primary-hover shadow-md hover:shadow-lg rounded-full transition-all duration-300 flex items-center transform hover:-translate-y-0.5 whitespace-nowrap"
            onClick={() => {
              btnName === "Sign In"
                ? setBtnName("Sign Out")
                : setBtnName("Sign In");
          ...
          <li className="px-4 font-bold text-gray-800 flex items-center whitespace-nowrap">{loggedInUser}</li>
        </ul>
      </div>
    </div>
  );
